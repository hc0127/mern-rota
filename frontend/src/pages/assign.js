...
import { npUpd } from "../store/Actions/BasicAction";
...
class Assign extends Component {
  ...
  save = () => {
    ...
    if (isEditable) {
      ...
      let newAssign = [];
      let month =
        selYear + "-" + (selMonth < 10 ? +"0" + String(selMonth) : selMonth);
    ...
    basic.nurses.map((nurse, index) => {
      if (nurse._id == row.nurse_id) {
        ...
        if (this.state.assigns[row.day - 1].duty_end != "NA") {
          this.state.assigns[row.day - 1].hour = Math.abs(
            this.state.assigns[row.day - 1].duty_end.split(":")[0] -
              this.state.assigns[row.day - 1].duty_start.split(":")[0]
          );
        }
      }
    });
